...
float32 SampleAndAggregate(pd_shared3p int64 [[1]]T, pd_shared3p bool[[1]]m){
    ...
    print(subset_size);
    pd_shared3p int64 [[2]] subsets(L, subset_size);
    subsets = reshape(T, L, subset_size);
...
float32 differentiallyPrivateQuery(pd_shared3p int64[[2]] V, pd_shared3p bool[[1]] m, int64 eps, pd_shared3p int64[[2]] B){
    ...
    pd_shared3p bool[[2]] mask = reshape(m,n,1);
    uint s = 0; uint t = 0;
...
//    B[2,:] = {3, 102};
    // for testing :
    pd_shared3p int64[[2]] V(n,2);
    V[:,0] = randomize(V[:,0]);
    V[:,0] = (int64)((uint)V[:,0]%L);
    V[:,1] = 6;

    pd_shared3p int64[[2]] B(n,2);
    B[:,0] = randomize(B[:,0]);
    B[:,0] = (int64)((uint)B[:,0]%L);
    B[:,1] = 1000;

    pd_shared3p bool[[1]] m(n);
    m = true;
    float32 result = differentiallyPrivateQuery(V, m, (int64)E, B);
